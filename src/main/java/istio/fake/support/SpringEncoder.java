...
public class SpringEncoder implements Encoder {
			...
			for (HttpMessageConverter<?> messageConverter : this.messageConverters
				...
				if (messageConverter.canWrite(requestType, requestContentType)) {
					...
					try {
						...
						throw new FakeException("Error converting request body", ex);
					}
					// clear headers
					request.headers(null);
					// converters can modify headers, so update the request
					// with the modified headers
					request.headers(Util.getHeaders(outputMessage.getHeaders()));
	...
	private final class FeignOutputMessage implements HttpOutputMessage {
		...
		private FeignOutputMessage(RequestTemplate request) {
			this.httpHeaders = Util.getHttpHeaders(request.headers());
		}
