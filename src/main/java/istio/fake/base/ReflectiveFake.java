...
import istio.fake.base.template.UriUtils;
import istio.fake.openfake.Target;

public class ReflectiveFake extends Fake {

    private final ParseHandlersByName targetToHandlersByName;
    private final InvocationHandlerFactory factory;
    private final QueryMapEncoder queryMapEncoder;

    public ReflectiveFake(ParseHandlersByName targetToHandlersByName, InvocationHandlerFactory factory,
    ...
    public <T> T newInstance(Target<T> target) {
        ...
        for (Method method : target.type().getMethods()) {
            if (method.getDeclaringClass() == Object.class) {
                ...
                methodToHandler.put(method, nameToHandler.get(Fake.configKey(target.type(), method)));
            }
        }
        ...
        public Map<String, InvocationHandlerFactory.MethodHandler> apply(Target key) {
            ...
            for (MethodMetadata md : metadata) {
                ...
                if (!md.formParams().isEmpty() && md.template().bodyTemplate() == null) {
                    buildTemplate = new BuildFormEncodedTemplateFromArgs(md, encoder, queryMapEncoder, httpRequestHeaderHolder);
                } else if (md.bodyIndex() != null) {
                    buildTemplate = new BuildEncodedTemplateFromArgs(md, encoder, queryMapEncoder, httpRequestHeaderHolder);
                } else {
                    buildTemplate = new BuildTemplateByResolvingArgs(md, queryMapEncoder, httpRequestHeaderHolder);
                }
    ...
    private static class BuildTemplateByResolvingArgs implements RequestTemplate.Factory {
        ...
        private BuildTemplateByResolvingArgs(MethodMetadata metadata, QueryMapEncoder queryMapEncoder, HttpRequestHeaderHolder httpRequestHeaderHolder) {
            ...
            for (Entry<Integer, Class<? extends Param.Expander>> indexToExpanderClass : metadata
        ...
        public RequestTemplate create(Object[] argv) {
            ...
            // 自定义header 如链路追踪，用户id等
            Map<String, Object> headerMap = this.httpRequestHeaderHolder.getHeaderMap();
            if (headerMap != null) {
    ...
    private static class BuildFormEncodedTemplateFromArgs extends BuildTemplateByResolvingArgs {
        ...
        private BuildFormEncodedTemplateFromArgs(MethodMetadata metadata, Encoder encoder,
                                                 QueryMapEncoder queryMapEncoder, HttpRequestHeaderHolder httpRequestHeaderHolder) {
            super(metadata, queryMapEncoder, httpRequestHeaderHolder);
